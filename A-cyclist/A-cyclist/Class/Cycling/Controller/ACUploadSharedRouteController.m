...
@interface ACUploadSharedRouteController () <UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIScrollViewDelegate>
/** 路线类别 */
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    [picker dismissViewControllerAnimated:YES completion:^() {
        ...
        if (ACImageClickedFirst == self.imageClicked) {
            self.leftImageView.image = [info objectForKey:@"UIImagePickerControllerOriginalImage"];
        } else if (ACImageClickedSecond == self.imageClicked) {
            self.middleImageView.image = [info objectForKey:@"UIImagePickerControllerOriginalImage"];
        } else if (ACImageClickedThird == self.imageClicked) {
            self.rightImageView.image = [info objectForKey:@"UIImagePickerControllerOriginalImage"];
        } else {
            DLog(@"未知错误");
        }
    }];
